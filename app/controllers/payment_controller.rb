...
class PaymentController < ApplicationController
	...
	def online_transaction_handler #Method to return JSON dataobject,to pass in inegenico js for Online Transaction.
		...
		if request.post?
			begin
				...
				if @config_data['siDetailsAtMerchantEnd'] == 'false' and @config_data['enableSI'] == 'true'
	...
	def admin_handler #Method that would create a JSON file and store config values.
	...
	def offline_verification_handler #Method that would call an API for offline verification.
		if request.get?
			begin
				Configdata.setData
				@config_data = Configdata.getData
	...
	def refund_handler  #Method that would call an API for refund.
		if request.get?
			begin
				Configdata.setData
				@config_data = Configdata.getData
	...
	def s2s_handler # Method for S2S 
		...
		begin
			...
			if @config_data == 403
				...
				if data[15] == String(OpenSSL::Digest.new('sha512', data_string))
					@status = 1
				else
					@status = 0
				end 
	...
	def get_datastring(data) #Method that would written a piped string to the caller
		return  data['mrctCode'] + "|" + data['txn_id']+ "|" + data['amount']+ "|" + data['accNo']+ "|" \
	...
	def get_hash_object(hashdata, data, config_data) #Method that would return a hash object to the caller.
		string_to_bool = { "true"=>true, true=>true, "false"=>false, false=>false }
		prepared_object = { :tarCall => false,
							...
							:consumerData => { :deviceId => 'WEBSH2', #//possible values 'WEBSH1', 'WEBSH2' and 'WEBMD5'
								...
								:checkoutElement => if string_to_bool[config_data['embedPaymentGatewayOnPage']]== true then '#ingenico_embeded_popup' else '' end,
		...
		elsif string_to_bool[config_data['enableSI']] and !string_to_bool[data['siDetailsAtMerchantEndCond']]
			prepared_object[:consumerData].merge!(
				:debitStartDate => data['debitStartDate'].split('-').reverse.join('-'),
				:debitEndDate => data['debitEndDate'].split('-').reverse.join('-'),
				:maxAmount => data['maxAmount'],
